  ...
  async (
    ...
    try {
      ...
      return rejectWithValue(
        error.response?.data?.error || "Failed to load projects",
      );
    }
  },
);
  ...
  async (
    {
      ...
      id: number;
      changes: Partial<
        Pick<Project, "name" | "description" | "colorCode" | "status">
      >;
    },
    { rejectWithValue },
  ) => {
...
export const fetchProjectMembers = createAsyncThunk(
  ...
  async (
    { projectId, page = 1 }: { projectId: number; page?: number },
    { rejectWithValue },
  ) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.error || "Failed to load members",
      );
    }
  },
);
  ...
  async (
    {
      ...
      membershipId,
      role,
    }: { projectId: number; membershipId: number; role: string },
    { rejectWithValue },
  ) => {
    try {
