...
import { fetchMe, loginUser, loginWithGoogle, logoutAllDevices, logoutUser, refreshUser, registerUser, updateProfile } from './asyncThunks';
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // LOGOUT
      .addMatcher((action) => action.type === logoutUser.fulfilled.type || action.type === logoutAllDevices.fulfilled.type, (state) => {
        state.user = null;
        state.isAuthenticated = false;
        state.status = 'idle';
        sessionStorage.removeItem('access_token');
      ...
      // REFRESH REJECTED
      .addMatcher(
        (action) => action.type === refreshUser.rejected.type || action.type === fetchMe.rejected.type,
        (state) => {
          state.initialized = true;
        }
      )
